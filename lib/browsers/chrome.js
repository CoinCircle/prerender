...
/**
 * Basic, fast, insecure hashing function
 */
...
/**
 * /deepfates/memery.jpg?a=20.23.345.2 => 'jpg'
 * 
 * 
 * /deepfates/memery => ''
 */
const getFileExt = (url) => {
	try {
		...
		console.log('Error getting file extension for', url, err)
		return '';
	}
  }

const getS3Key = (originalUrl) => {
	const url = new URL(originalUrl);
	/** sub folder in s3 to store url */
	const folder = url.host;
	// generate key for object with basic hash of url path
	const key = simpleHash(`${url.pathname}${url.hash}`);
	const fileExtension = getFileExt(url.pathname || '');
	const extension = fileExtension ? `.${fileExtension}` : '';
	return `uploads/${folder}/${key}${extension}`;
}

async function uploadToS3(params) {
	let pass = new Stream.PassThrough();

	const key = getS3Key(params.response.url)

	const uploadParams = {
		Bucket: s3Config.bucket,
		Key: key,
		Body: pass,  
		ContentType: params.response.mimeType,
...
chrome.cacheAsset = async (params, tab) => {
	try {
		if (!params.response?.url) {
			return;
		}
		// check if we should cache the asset
		if (params.response?.status < 200 || params.response?.status >=300) { 
			return;
		}
		const assetURL = new URL(params.response.url);
		const isFont = /\.(?:woff2|woff|ttf|otf)$/.test(assetURL.pathname);
		const isStyleSheet = params.type && params.type.toLowerCase() === 'stylesheet';
		const isImage = params.type && params.type.toLowerCase() === 'image';
		const isMedia = params.type && params.type.toLowerCase() === 'media';
		// only cache if font, stylesheet, image, or media type
		if (!isFont && !isStyleSheet && !isImage && !isMedia) {
			return;
		}
		const isHTTPS = (assetURL.protocol || '').toLowerCase() === 'https:';
		// only cache if https 
		if (!isHTTPS) {
			return;
		}

		const isCoincircleCDN = /cdn\.coincircle\.com/.test(params.response.url);
		const currSite = new URL(tab.prerender.url);
		/** 
		 * asset url from same domain of site the page belongs to. 
		 * 
		 * these urls will commonly be replaced by OG url in JS after html returned which makes replacing them in first place useless
		 * */
		const isFromSameDomain = currSite.hostname && assetURL.hostname && currSite.hostname === assetURL.hostname;
		if (isCoincircleCDN || isFromSameDomain) {
			// dont cache if already in CC cdn or from same domain as the site using the url
			return;
		}
		...
		if (!res.ok) {
			throw Error('Got bad status code fetching url');
		}

		console.log('Caching asset to s3 (url, type, mimetype):', params.response.url, params.type, params.response.mimeType)
		res.body.pipe(await uploadToS3(params));
		chrome.urlsToReplace = { ...chrome.urlsToReplace, [params.response.url]: `${s3Config.cdnUrl}${getS3Key(params.response.url)}`}
	} catch (err) {
...
chrome.setUpEvents = async function (tab) {
	...
	Network.responseReceived(async (params) => {
		...
		// Cache external assets to s3 and replace urls with our cdn urls
		await chrome.cacheAsset(params, tab);
